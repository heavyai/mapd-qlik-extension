...
var { createFilters, createSQLFilterStatement, parseDataType } = proxyquire(
  '../src/filters-handler',
  {
...
describe('Create SQL Filter Statement', function() {
  ...
  it('Should create a filter statement from one column and three filters', function() {
    ...
    assert.equal(
      createSQLFilterStatement(input),
      "((column='filter1' OR column='filter2' OR column='filter3'))"
    );
  });
  ...
  it('Should create a filter statement from two columns and two filters', function() {
    ...
    assert.equal(
      createSQLFilterStatement(input),
      "((column='filter1' OR column='filter2') AND (column2='filter3' OR column2='filter4'))"
    );
  });
});
...
describe('Parse Data Type Statement', function() {
  it('Should parse HISTOGRAM data', function() {
    ...
    var expectedResult = {
      parsedValues: [
        'amount >= -26500  and amount <  -19000',
        'amount >= -19000  and amount <  -11500',
        'amount >= -11500  and amount <  -4000',
        'amount >= -4000  and amount <  3500'
      ],
      parsedKey: 'amount'
    };

    assert.equal(
      JSON.stringify(parseDataType(type, key, values)),
  ...
  it('Should parse INDIVIDUAL_SELECTIONS data', function() {
    ...
    var values = ['Maintenance of Way Employees', 'Texas Medical Assn'];

    var expectedResult = {
      parsedValues: [
        "recipient_name='Maintenance of Way Employees'",
        "recipient_name='Texas Medical Assn'"
      ],
